...
const validateAndSanitizeCheckoutForm = (data: {
  ...
  data.country = !isEmpty(data.country) ? data.country : 'US';
  ...
  data.paymentMethod = !isEmpty(data.paymentMethod)
    ? data.paymentMethod
    : 'square_credit_card';
  ...
  const addErrorAndSanitizedData = (
    ...
    // if (type === 'phone' && !validator.isMobilePhone(data[fieldName])) {
    //   errors[fieldName] = `${errorContent} is not valid`;
    // }
  ...
  addErrorAndSanitizedData('phone', 'Phone number', 10, 15, 'phone', false);
